...
class _SkinConductivityScreenState extends State<SkinConductivityScreen> {
  ...
  void updateStats() {
    ...
    setState(() {
      double? a = (separateRecordings[currSession]
          .map((e) => e.skinConductivity)).reduce((a, b) {
        return min(a!, b!);
      });
      double? b = (separateRecordings[currSession]
          .map((e) => e.skinConductivity)).reduce((a, b) {
        return max(a!, b!);
      });
      double? c = separateRecordings[currSession]
          .map((e) => e.skinConductivity!)
          .average;
      sessionData = [a!, b!, c];
    });
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text("Skin Conductivity"),
        elevation: 0,
        backgroundColor: const Color(0xFF00564D),
      ),
      body: SkinConductivityChart(),
    );
  }
